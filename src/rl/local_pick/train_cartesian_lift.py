...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SAC for Cartesian local pick with lift-focused reward.")
    parser.add_argument("--timesteps", type=int, default=500_000)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--model-path", type=Path, default=Path("models/local_pick_cartesian_lift_sac"))
...
def _make_env(config: LocalPickConfig):
    ...
    return Monitor(LocalPickCartesianLiftEnv(config=config, render_mode=None))
...
def main():
    ...
    env.close()
    print(f"Saved lift-focused Cartesian local pick model to {args.model_path}")
