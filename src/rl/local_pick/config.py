...
class LocalPickConfig:
    ...
    fixed_object_position: tuple[float, float, float] | None = (0.4, -0.15, 0.13)
    object_xy_noise: float = 0.0
    approach_xy_noise: float = 0.0
    joint_noise: float = 0.0

    # Scene geometry.
    table_x_min: float = 0.38
    table_x_max: float = 0.62
    table_y_min: float = -0.24
    table_y_max: float = -0.08
    object_z: float = 0.13
    approach_height: float = 0.16
    side_approach_offset: float = 0.10
    side_grasp_height: float = 0.03
    lift_success_z: float = 0.20

    # Control.
    max_episode_steps: int = 75
    action_scale: float = 0.02
    frame_skip: int = 20
    gripper_close_step: int = 20
