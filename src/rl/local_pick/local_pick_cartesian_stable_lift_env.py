...
class LocalPickCartesianStableLiftEnv(LocalPickCartesianLiftEnv):
    """Lift-focused Cartesian env with slower motion and stability shaping."""

    def __init__(self, config: LocalPickConfig | None = None, render_mode: str | None = None):
        config = config or LocalPickConfig()
        config.action_scale = min(config.action_scale, 0.01)
    ...
    def step(self, action):
        self._prev_object_pos = self.data.xpos[self.object_body_id].copy()
        self._prev_object_lift = float(max(0.0, self._prev_object_pos[2] - self.initial_object_z))
    ...
    def _compute_reward(self, cart_action: np.ndarray, gripper_cmd: float) -> tuple[float, dict[str, float]]:
        ...
        if valid_grasp:
            lift_progress_reward = 80.0 * lift_delta
            lift_fraction = min(object_lift / self.config.min_lift_for_success, 1.0)
            lifted_hold_reward = 2.0 * lift_fraction
            if object_lift > 0.005:
                stability_penalty = 12.0 * object_motion

        invalid_lift_penalty = 0.0
        if object_lift > 0.006 and not valid_grasp:
            invalid_lift_penalty = 8.0 * min(object_lift / self.config.min_lift_for_success, 1.0)

        if valid_grasp and object_lift >= self.config.min_lift_for_success and object_motion < 0.01:
    ...
    def _is_success(self) -> bool:
        return self._stable_lift_count >= 10
