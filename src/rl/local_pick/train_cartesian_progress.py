...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SAC for Cartesian progress-reward local pick.")
    parser.add_argument("--timesteps", type=int, default=2_000_000)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--model-path", type=Path, default=Path("models/local_pick_cartesian_progress_sac_2m"))
...
def _make_env(config: LocalPickConfig):
    ...
    return Monitor(LocalPickCartesianProgressEnv(config=config, render_mode=None))
...
def main():
    ...
    env.close()
    print(f"Saved Cartesian progress model to {args.model_path}")
