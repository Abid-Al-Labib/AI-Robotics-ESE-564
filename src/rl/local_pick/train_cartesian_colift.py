...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SAC for Cartesian co-lift local pick.")
    parser.add_argument("--timesteps", type=int, default=1_200_000)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--model-path", type=Path, default=Path("models/local_pick_cartesian_colift_sac_1p2m"))
...
def _make_env(config: LocalPickConfig):
    ...
    return Monitor(LocalPickCartesianCoLiftEnv(config=config, render_mode=None))
...
def main():
    ...
    env.close()
    print(f"Saved Cartesian co-lift model to {args.model_path}")
