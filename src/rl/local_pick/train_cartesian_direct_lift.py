...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SAC for direct Cartesian local pick lift.")
    parser.add_argument("--timesteps", type=int, default=2_000_000)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--model-path", type=Path, default=Path("models/local_pick_cartesian_direct_lift_sac_2m"))
    parser.add_argument("--pretrained", type=Path, default=None, help="Optional SAC .zip to continue from.")
    parser.add_argument("--n-envs", type=int, default=8)
    parser.add_argument("--object-noise", type=float, default=0.005)
    parser.add_argument("--approach-noise", type=float, default=0.003)
    parser.add_argument("--approach-z-noise", type=float, default=0.003)
    parser.add_argument("--joint-noise", type=float, default=0.003)
...
def _make_env(config: LocalPickConfig):
    ...
    return Monitor(LocalPickCartesianDirectLiftEnv(config=config, render_mode=None))
...
def main():
    ...
    env.close()
    print(f"Saved direct Cartesian lift model to {args.model_path}")
